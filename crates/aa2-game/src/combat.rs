...
pub fn build_team(
    ...
    for hero_name in &player.heroes {
        ...
        team.push((config, Vec2::new(pos.0, pos.1)));
    }
...
pub fn run_combat(
    team_a: &[(UnitConfig, Vec2)],
    team_b: &[(UnitConfig, Vec2)],
    seed: u32,
) -> (Option<u8>, u32, u32) {
    ...
    for (config, pos) in team_a {
        units.push(Unit::from_config(config, id, 0, *pos));
        id += 1;
    }
    for (config, pos) in team_b {
        let mirrored = crate::player::mirror_position(pos.x, pos.y);
        units.push(Unit::from_config(config, id, 1, Vec2::new(mirrored.0, mirrored.1)));
        id += 1;
    }
...
pub fn run_all_matchups(
    ...
    for (i, matchup) in matchups.iter().enumerate() {
        ...
        let team_a = player_a
            .map(|p| build_team(p, hero_defs, ability_defs, hero_level))
            .unwrap_or_default();
